...
class Command extends Base {
    ...
    _execute(env) {
        ...
        return this.execute(env)
            ...
            .then( (response) => {
                this._cooldown.shouldSetCooldown(response)
                this._cooldown.setCooldown(this.library.message.getAuthorID(msg) );
    ...
    sendBotPerms(channel, permissions = [] ) {
        if (permissions.length === 0) {
            ...
            permissions = this.utils.missingChannelPerms(channel, member, this.permissions.bot);
        }
